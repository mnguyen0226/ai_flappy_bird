...
class Pipe:
    ...
    def __init__(self, x):
        ...
        self.PIPE_BOTTOM = PIPE_IMG  # store the flip pipe
        self.passed = False
        self.set_height()
...
def main(genomes, config):
    ...
    while run:
        ...
        for pipe in pipes:
            for x, bird in enumerate(birds):
                if pipe.collide(bird):
                    ge[x].fitness -= 1 # if hit the pipe then remove 1 from fitness score
                    birds.pop(x)
                    nets.pop(x)
                    ge.pop(x)

                if not pipe.passed and pipe.x < bird.x: # check if the bird pass by the pipe
                    pipe.passed = True
                    add_pipe = True
        ...
        for x, bird in enumerate(birds):
            if bird.y + bird.img.get_height() >= 730 or bird.y < 0: # we hit the floor or jump over the screen
                birds.pop(x)
                nets.pop(x)
                ge.pop(x)
